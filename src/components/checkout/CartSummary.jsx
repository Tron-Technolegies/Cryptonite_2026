            ...
            <div key={item.id || index} className="flex gap-4 group items-start">
              <div className="h-16 w-16 bg-gray-800 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-gray-800/80 transition-colors">
                {item.product?.image || item.bundle?.image ? (
                  <img
                    src={getImageUrl(item.product?.image || item.bundle?.image)}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start gap-2">
                  <h3
                    className="text-md font-medium truncate py-0.5"
                    title={item.product?.name || item.bundle?.name}
                  >
                ...
                <div className="flex justify-between items-center mt-3">
                  ...
                  <p className="font-mono text-sm text-white">
                    $
                    {(
                      (item.product?.price || item.bundle?.price || 0) * (item.quantity || 1)
                    ).toLocaleString()}
                  </p>
                </div>
              </div>
